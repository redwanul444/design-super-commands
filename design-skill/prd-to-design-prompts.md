...
When no Design Guide exists, this skill now auto-derives design context before generating prompts:

1. **Domain Research (minimal)**: Run `analyze-design` search with PRD keywords to get recommended style, color palette, font pairing, and UX patterns for the industry.
2. **Design Pillars (auto-derived)**: Map core features to reference apps — Stripe (dashboard), LinkedIn (feed), Reddit (comments), Product Hunt (voting), Slack (chat), Shopify (e-commerce), Typeform (forms/wizard).
3. **Component Patterns (auto-derived)**: Extract from feature keywords — card patterns for lists, form patterns for inputs, table patterns for admin, stats patterns for dashboards.

This produces significantly better HTML output because the LLM has design philosophy context to ground each page prompt in, rather than just raw color hex codes.
