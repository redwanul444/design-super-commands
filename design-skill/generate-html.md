...
| Priority | Condition | Path | Skill |
|----------|-----------|------|-------|
| 1 | `GEMINI_API_KEY` is set | Primary (A1) | `generate-html-gemini/SKILL.md` |
| 2 | `templates/html/*.html` exist | Template (A3) | `templates-to-html/SKILL.md` |
| 3 | Neither available | Fallback (A2) | `prompts-to-html/SKILL.md` |

**A3 (Templates)** is now the preferred non-Gemini path — it fills pre-built, production-tested HTML layouts with design tokens. This produces more consistent, responsive, accessible output than free-form Claude HTML generation (A2). Use A2 only for pages with no matching template.
...
1. Check `GEMINI_API_KEY` environment variable
   - If set → use A1 (`generate-html-gemini`)
2. If Gemini unavailable, check for `DESIGN_TOKENS.json` + `templates/html/*.html`
   - If both exist → use A3 (`templates-to-html`) — fills tokens into pre-built layouts
3. If neither available → use A2 (`prompts-to-html`) — last-resort free-form generation
4. Read `DESIGN_STATUS.md` to determine selected variation
5. Delegate to appropriate generation skill
6. Validate output meets all MANDATORY requirements (device frames, navigation, dark/light mode, responsive breakpoints, accessibility)
...
- `.claude/skills/design/generate-html-gemini/SKILL.md` -- Primary path (Gemini API, A1)
- `.claude/skills/design/templates-to-html/SKILL.md` -- Preferred non-Gemini path (templates, A3)
- `.claude/skills/design/prompts-to-html/SKILL.md` -- Last-resort fallback path (free-form Claude, A2)
...
- `v2/scripts/export-design-tokens.js` -- Generates DESIGN_TOKENS.json (prerequisite for A3 path)
