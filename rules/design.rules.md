...
## Token Export (MANDATORY after HTML generation)
- Export DESIGN_TOKENS.json from DESIGN_SYSTEM.md using:
  ```bash
  node .claude/v2/scripts/export-design-tokens.js .claude-project/<project>/design/
  ```
- DESIGN_TOKENS.json provides machine-readable tokens for downstream phases (frontend, user-stories)
- tokens.css provides CSS custom properties for the generated HTML
- Both files must pass gate validation

## Responsive HTML Output (MANDATORY)
- All generated HTML must include responsive breakpoints:
  - `sm:` for tablet (768px+)
  - `md:` for medium (1024px+)
  - `lg:` for large desktop (1280px+)
- Desktop container must use `max-w-[1440px]` or `max-width: 1440px`
- Mobile-first approach: base styles for 375px, overrides for larger screens
- No horizontal scroll on any breakpoint

## Dark/Light Mode (MANDATORY)
- All HTML must support `data-theme="light|dark"` attribute on `<html>`
- Colors must be defined as CSS custom properties (`var(--color-*)`)
- Dark mode palette must be declared in `:root [data-theme="dark"]` block
- Theme toggle must be present in the navbar
- `prefers-color-scheme` must be respected on initial load
...
- No empty links or buttons — every <a> and <button> must have text or aria-label
